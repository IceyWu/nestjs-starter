...
async function release(plop) {
  ...
  const urls = remotes.map((r) => {
    return r.refs.push.replace('git@github.com:', 'https://github.com/').replace('.git', '')
  })
      ...
      prompts: [
        {
          ...
          default: 'patch',
          message: '你希望发布一个什么版本?',
          choices: ['patch', 'minor', 'major', 'prepatch', 'premajor', 'preminor', 'prerelease'],
        },
      ],
      actions(answer) {
        const { type } = answer
        execSync(`npx changelogen --${type} --release && git push --follow-tags`, {
          stdio: 'inherit',
        })
        return []
      },
